...
max_filtration_value = filtration_size;
    %Recode Inf as NaN
    %exclude the longest B0 bar since it is not meaningful
    outmat=outmat(outmat(:,3)~=max_filtration_value,:);
    outmat(isinf(outmat))=NaN;% if longest extends to Inf: in JAVAplex and TDA alpha
    %length of intervals for each dimension
    len=outmat(:,3)-outmat(:,2);
    ...
    %F3,F11 sum of intervals
        F_3=sum(len(outmat(:,1)==0),'omitnan');%sum up lengths of B0 bars
        F_11=sum(len(outmat(:,1)==1),'omitnan');% sum up lengths of B1 bars
    %F4,F12 mean of intervals
        F_4=F_3./sum(len(outmat(:,1)==0)~=0,1);
        F_12=F_11./sum(len(outmat(:,1)==1)~=0,1);
    %sort the interval lengths in order for each dimension
        %[~,indices0]=sort(len(outmat(:,1)==0),'descend');
        [~,indices1]=sort(len(outmat(:,1)==1),'descend');
    ...
    %F1: second longest Betti 0 bar length
        F_1=max(len(outmat(:,1)==0),[],'omitnan');
        %F_1= ordered_len(2,1);
    %F2: third longest Betti 0 bar length
        F_2=max(len(len(outmat(:,1)==0)<F_1),[],'omitnan');
        %F_2= ordered_len(3,1);
    
    %use the indices from sorting to trace back to the longest
    %betti 1 bar
    %F5:onset value of longest betti 1 bar: row no. is indices(1,2)
        outmat1=outmat(outmat(:,1)==1,:);
        F_5=outmat1(indices1(1),2);
    %F6:longest Betti 1 bar length
        %F_6=outmat1(indices1(1),3)-outmat1(indices1(1),2);
        F_6=max(len(outmat(:,1)==1),[],'omitnan');
        %F_6=ordered_len(1,2);
    %F7: smallest onset value of Betti1 bar longer than 1.5A
        l7=find(len(outmat(:,1)==1)>1.5,sum((outmat(:,1)==1)));%returns indice of elements that satisfy condition
        if isempty(l7)==0
            F_7=min(outmat1(l7,2));
        else
            F_7=0;
        end 
    ...
    %divided by number of such intervals
        l8=find(len(outmat(:,1)==1)>1.5,sum((outmat(:,1)==1)));
        if isempty(l8)==0
            F_8=mean(0.5*(outmat1(l8,2)+outmat1(l8,3)));
        else
            F_8=0;
        end 
    ...
    %divided by number of atoms
        ...
        %no_B1_4555=length(l);
        l9=(outmat1(:,2)>=4.5)&(outmat1(:,3)<=5.5);
        F_9=sum(l9)/natom;
    %F10:The no. of B1 bars locate at [3.5,4.5]A and [5.5,6.5]A
    %divided by no. of atoms
        l10=(outmat1(:,2)>=3.5&outmat1(:,3)<4.5)|(outmat1(:,2)>5.5&outmat1(:,3)<=6.5);
        F_10=sum(l10)/natom;
    %F13: The onset of first B2 bars(column 5 of outmat) that end after given
    %number: gives info about birth and death of cavities
%                 if sum(outmat(:,1)==2)==0
%                     F_13=0; % ie there are no B2 bars
%                 else %ie there are B2 bars ( 6 columns in outmat)
%                     l13=find(outmat(outmat(:,1)==2,3)>10,length(outmat(:,1)==2));
%                         if isempty(l13)==0
%                             F_13=(outmat(outmat(:,1)==2)==min(outmat(l13,6)),2));
%                         else
%                             F_13=0;% or set as 0?
%                         end
%                 end
        ...
        %mean_len(dim0)...
        features=[F_1,F_2,F_3,F_4,F_5,F_6,F_7,F_8,F_9,F_10,F_11,F_12];
end
