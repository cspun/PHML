...
sMAT<-read.csv("~/Desktop/Rfiles-SEQ-AB/stageIII-ML/InputFilesSMAT/RDC-TDA-ALPHA-BN_SMAT.csv",header=T)
sMAT=sMAT[,-1]#remove first column after reading in
...
train.dat<-dat[tr_ind,c(1,(tr_ind+1)),drop=F]#gives a 360*360 mat
test.dat<-dat[-(tr_ind),c(1,(tr_ind+1)),drop=F]# gives a 90*360 mat
...
data2libsvmTR=function(mat,filename){
    ...
    write.matrix.csr(xs.tr, y=y.tr, file='out-train.txt') 
...
data2libsvmTE=function(mat){
  ...
  # write the output libsvm format file 
  write.matrix.csr(xs.te, y=y.te, file="out-test.txt") 
  
}

#create sparse format data
data2libsvmTR(train.dat)
