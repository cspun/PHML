...
# data <- train.dat
# test <- test.dat
# y <- as.factor(make.names(data$classLab))
# 
...
#2 hidden units
# tic()
# fit1<- nn.rep(rep = 30, classLab ~ ., data = training,
#               size = 2, linout = F, trace = FALSE, MaxNWts=10000 )
# toc()
# 
# #4 hidden units
# tic()
# fit2 <- nn.rep(rep = 30, classLab ~ ., data = training,
...
#                size = 32, linout = F, trace = FALSE, MaxNWts=20000 )
# toc()

#64 hidden units
# tic()
# fit6 <- nn.rep(rep = 30, classLab ~ ., data = training,
...
fit <- nn.rep(rep = 30, classLab ~ ., data = training,
              size = best_unit, linout = F, trace = FALSE, MaxNWts=20000 )
toc()
#predict using test data "test"
pred.test <- predict(fit, newdata=test, type="class")
insampleclasspred <-  predict(fit, newdata=training, type="class")
confusionMatrix(insampleclasspred, training$classLab)
